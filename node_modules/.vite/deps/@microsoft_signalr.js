...
var XhrHttpClient = class extends HttpClient {
	...
	send(request) {
		...
		return new Promise((resolve, reject) => {
			...
			if (request.content === "") request.content = void 0;
			if (request.content) {
				if (isArrayBuffer(request.content)) xhr.setRequestHeader("Content-Type", "application/octet-stream");
...
var MessageBuffer = class {
	...
	_shouldProcessMessage(message) {
		if (this._waitForSequenceMessage) {
			if (message.type !== MessageType.Sequence) return false;
			else {
				this._waitForSequenceMessage = false;
				return true;
			}
		...
		try {
			...
			if (error) {
				if (error instanceof HttpError) {
					if (error.statusCode === 404) this._logger.log(LogLevel.Trace, "(LongPolling transport) A 404 response was returned from sending a DELETE request.");
...
var WebSocketTransport = class {
	...
	_close(event) {
		...
		if (this.onclose) {
			if (this._isCloseEvent(event) && (event.wasClean === false || event.code !== 1e3)) this.onclose(/* @__PURE__ */ new Error(`WebSocket closed with status code: ${event.code} (${event.reason || "no reason given"}).`));
...
var HttpConnection = class {
	...
	_resolveTransportOrError(endpoint, requestedTransport, requestedTransferFormat, useStatefulReconnect) {
		...
		if (transport === null || transport === void 0) {
			...
			if (endpoint.transferFormats.map((s) => TransferFormat[s]).indexOf(requestedTransferFormat) >= 0) {
